<!--
Standard PR 模板(适用于 feature / refactor / perf / 任何改了行为或接口的 PR)
=========================================================================

- 适用条件:
  - 新功能 / 行为变更 / 重构 / 性能改动 / 任何跨模块的接口变化
  - 默认情况下,此模板的字段比 `?template=short.md` 多。
  - 如果你的改动会动 `AGENTS.md` / `.kiro/steering/` / `templates/` /
    `scripts/` / `.github/workflows/`,请切到 **harness_change.md**:
    `?template=harness_change.md`
- 不适用的字段写 `N/A`,**不要整段删除**——保留空字段让 reviewer
  一眼看到这项"被考虑过"。
- 相关文件:
  - /AGENTS.md
